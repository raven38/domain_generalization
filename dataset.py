...
class DG_Dataset(Dataset):
    def __init__(self, root_dir, domain, split, get_domain_label=False, get_cluster=False, color_jitter=True, min_scale=0.8, classes=None):
        ...
        self.images = list(compress(self.images, mask))
        self.domains = self.domains[mask]
        self.clusters = self.clusters[mask]
        self.labels = list(compress(self.labels, mask))
    ...
    def load_dataset(self):
        ...
        classes, class_to_idx = self.find_classes(str(self.root_dir / self.domain[0]))
        self.num_class = len(classes)
        for i, item in enumerate(self.domain):
            path = self.root_dir / item 
            samples = make_dataset(str(path), class_to_idx, IMG_EXTENSIONS)
